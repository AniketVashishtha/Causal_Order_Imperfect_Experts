"""
Run the quadruplet-based causal discovery experiment on a benchmark graph.

This uses the same subgroup voting strategy as run_triplet but with 4-node
subgroups (C(n,4) combinations) instead of 3-node triplets.

Usage:
    python -m causal_discovery.run_quadruplet --graph child --subgraph-model gpt-4o-mini --expert-model gpt-4o
...
def main():
    parser = argparse.ArgumentParser(
        description="Quadruplet-based LLM causal discovery")
    ...
    parser.add_argument("--subgraph-model", type=str, default="gpt-4o-mini",
                        help="Weaker model for quadruplet subgraph orientation (default: gpt-4o-mini)")
    ...
    print(f"Subgroup size: 4 (quadruplet)")
